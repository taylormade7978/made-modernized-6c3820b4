...
//!
//! An [`EmissionPool`] holds the season's $MADE reward budget and emits rewards
//! to recipients within its remaining balance. Four invariants govern whether a
//! reward may be emitted:
//!
...
//!    balance; an emission that would overdraw the pool is rejected.
//! 3. **Low-pool warning** - the low-pool warning must be raised at 80%
...
//! (`reward.emitted`). This module is hand-written (it does not use
...
/// The EmissionPool aggregate: the $MADE reward budget a season draws down.
///
...
/// carries the state [`EmitRewardCmd`] validates: the remaining balance, whether
/// the emission schedule matches the mandated halving, whether the low-pool
/// warning threshold is configured, and whether the governance gate is resolved.
#[derive(Debug)]
...
impl EmissionPool {
    ...
    /// Low-pool-warning invariant: the warning must be raised at 80% depletion
    /// (advance notice), not at exhaustion.
    fn ensure_low_pool_warning_valid(&self) -> Result<(), DomainError> {
        if !self.low_pool_warning_valid {
            return Err(DomainError::InvariantViolation(format!(
                "emission pool '{}' low-pool warning is misconfigured; the warning must be \
                 raised at 80% depletion (advance notice), not at exhaustion",
                self.id
            )));
        }
        Ok(())
